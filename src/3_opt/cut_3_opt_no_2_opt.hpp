#ifndef TSP_3_OPT_CUT_NO_2_OPT_HPP
#define TSP_3_OPT_CUT_NO_2_OPT_HPP
...
template<typename cost_t, typename vertex_t>
int Cut3OptNo2Opt<cost_t, vertex_t>::selectCut(
    const int i,
    const int j,
    const int k,
    cost_t &change,
    ...
    //     0     1       2     3        4    5
    // [ k + 1 : i] + [ i + 1, j ] + [j + 1, k]
    // allows single 2 opt move when 2 consequent edges
...
template<typename cost_t, typename vertex_t>
void Cut3OptNo2Opt<cost_t, vertex_t>::applyCut(
    const int i,
    const int j,
    const int k,
    const int patch_ordinal,
