...
"$SRC_DIR/main_.exe" "funky" "5_opt" 3 "$PROBLEMS_DIR/263.txt" "$HISTORY_DIR/263/tsp/263" 30  500 300 tsp 1 --no-history > "$RESULTS_DIR/263/tsp/3.txt" 2>&1 &
wait


echo "Solving 4 points..."
"$SRC_DIR/main_.exe" "funky" "5_opt" 4 "$PROBLEMS_DIR/263.txt" "$HISTORY_DIR/263/tsp/263" 30  500 300 tsp 1 --no-history > "$RESULTS_DIR/263/tsp/4.txt" 2>&1 &
wait


echo "Solving 263 points..."
"$SRC_DIR/main_.exe" "funky" "5_opt" 263 "$PROBLEMS_DIR/263.txt" "$HISTORY_DIR/263/shp/263" 1  500 300 shp 1 --no-history > "$RESULTS_DIR/263/shp/263_5opt.txt" 2>&1 &
# "$SRC_DIR/main_.exe" "funky" "5_opt" 263 "$PROBLEMS_DIR/263.txt" "$HISTORY_DIR/263/shp/263" 5  500 300 shp 1 --no-history > "$RESULTS_DIR/263/shp/263.txt" 2>&1 &
wait
