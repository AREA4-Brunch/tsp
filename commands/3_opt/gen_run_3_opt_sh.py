...
def main():
    ...
    for mode in MODES:
        ...
        print_sh_commands(MIN_N, MAX_N, algo, SINGLE_TEST_TIMEOUT_SEC,
                            NUM_RERUNS, RUNS_PER_HISTORY, mode,
...
def print_sh_commands(min_n, max_n, algo, timeout_secs, num_reruns, runs_per_history,
                      mode='shp', is_dist_pts_fmt=1, parallel_cap=None):
    print(f'mkdir -p "$RESULTS_DIR/{PROBLEM_NAME}/{mode}"')
    for n in range(min_n, max_n + 1):
        cmd = f'"$SRC_DIR/main.exe" "{algo}" {n} "$PROBLEMS_DIR/{PROBLEM_FNAME}"' \
